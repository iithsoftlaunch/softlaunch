...
-- 1. Add the Kill-Switch column
ALTER TABLE system_config ADD COLUMN IF NOT EXISTS picking_closed boolean not null default false;

-- 2. Allow the frontend to securely read the toggle
...
-- 3. Lock down the submit_picks function
CREATE OR REPLACE FUNCTION submit_picks(p_picks jsonb, p_kit_ct text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_account accounts%rowtype;
  ...
  v_closed boolean;
BEGIN
  -- NEW SECURITY LOCK: Mathematically reject any picks if the picking phase is closed
  SELECT picking_closed INTO v_closed FROM system_config WHERE id = 1;
  IF v_closed THEN
    RAISE EXCEPTION 'The picking phase has officially ended. No further submissions are allowed.';
  END IF;
...
AS $$
BEGIN
  -- Ensure email is not NULL to prevent SQL bypass
  IF NEW.email IS NULL OR NEW.email NOT LIKE '%@iith.ac.in' THEN
    RAISE EXCEPTION 'Only @iith.ac.in emails are allowed';
  END IF;
  RETURN NEW;
END;
$$;

-- 2. Fix the kit_ct Storage Parasite Vulnerability
CREATE OR REPLACE FUNCTION public.submit_picks(p_picks jsonb, p_kit_ct text)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
DECLARE
  v_account accounts%rowtype;
  v_group uuid := gen_random_uuid();
...
BEGIN
  ...
  -- STORAGE PARASITE FIX: Prevent massive 1GB recovery kits from freezing the database
  IF p_kit_ct IS NOT NULL AND length(p_kit_ct) > 5000 THEN
    RAISE EXCEPTION 'Recovery kit payload size exceeded limits.';
  END IF;
